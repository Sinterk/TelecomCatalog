...
export function Layout({ children }: Props) {
  ...
  function handleRoleChange() {
    if (confirm('¿Cambiar de rol?')) { clearRole(); window.location.reload() }
  }
  ...
  return (
    <div className="min-h-screen bg-slate-900 text-white flex flex-col">
      <header className="bg-slate-900 border-b border-slate-800 px-4 py-3 flex items-center gap-3 sticky top-0 z-30">
        ...
        {role && (
          <button type="button" onClick={handleRoleChange} title="Cambiar rol"
            className={`text-[10px] font-semibold px-2 py-0.5 rounded-full transition-colors ${role === 'jp' ? 'bg-violet-900/60 text-violet-300 hover:bg-violet-800/60' : 'bg-cyan-900/60 text-cyan-300 hover:bg-cyan-800/60'}`}>
            {role === 'jp' ? '💼 JP' : '📱 Técnico'}
          </button>
        )}
      </header>
