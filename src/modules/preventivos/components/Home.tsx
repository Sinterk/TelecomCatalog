      ...
      <div className="flex items-center justify-between">
        ...
        {role === 'tecnico' && (
          <button type="button" onClick={handleNew}
            className="bg-brand-600 hover:bg-brand-700 text-white text-sm font-semibold px-3 py-2 rounded-xl transition-colors">
            ➕ Nuevo
          </button>
        )}
      </div>

      {/* JP: bloque de importación */}
      {role === 'jp' && (
        <div className="bg-slate-800 rounded-2xl border border-slate-700 p-4 space-y-2">
          ...
          <p className="text-xs text-slate-400">Selecciona el archivo ZIP que te envió el técnico por WhatsApp:</p>
          <ImportZip onImported={(id) => navigate(`/preventivos/${id}`)} />
          <p className="text-[10px] text-amber-400/80">
            ⚠ En WhatsApp: el técnico debe enviar como <strong>Documento</strong> (clip 📎), no como imagen
          </p>
        </div>
      )}
        ...
        <button type="button" onClick={onSelect} className="flex-1 text-left min-w-0">
          <div className="text-sm font-semibold text-white">
            ...
            {record.cuadrante.comuna
              ? <span className="font-normal text-slate-400"> — {record.cuadrante.comuna}</span>
              : null}
          </div>
